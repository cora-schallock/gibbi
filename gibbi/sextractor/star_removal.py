...
# NOTE:  photutils appature use radians but sextractor outputs degrees
# Source: https://photutils.readthedocs.io/en/stable/api/photutils.aperture.EllipticalAperture.html#photutils.aperture.EllipticalAperture
...
def filter_objects(objects,min_star_class=0.1):
    ...
    return list(filter(lambda x: x["CLASS_STAR"]>=min_star_class,objects_to_keep))
...
def make_star_mask_from_sextractor_file(sex_path,fits_shape,min_star_class=0.1):
    ...
    objects_to_mask = filter_objects(objects,min_star_class=min_star_class)
